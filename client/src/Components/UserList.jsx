...
const UsersList = () => {
  ...
  const handleUserCheckboxChange = (event, user) => {
    ...
    if (userIndex >= 0) {
      ...
      const hasSameDomain = newSelectedUsers.some(
        (u) => u.domain === user.domain
      );
      if (!user.available) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await fetch(
        `http://localhost:3000/api/users?q=${search}${domain}${gender}${availability}&page=${currentPage}&limit=${pageSize}`
      );
  ...
  return (
    <div className="user-list-container">
      ...
      <div className="top-section">
        <p className="view-team" onClick={() => setIsModalOpen(!isModalOpen)}>
        ...
        {users.map((user) => (
          <div key={user.id} className="user-card">
            ...
            <div className="user-info">
              <div>
                <p>
                  <span className="bold">Domain</span> {user.domain}
                </p>
                <p>
                  <span className="bold">Gender</span> {user.gender}
                </p>
              </div>
              {user.available ? (
            ...
            <button
              ...
              onClick={() => handleUserCheckboxChange(this, user)}
            >
